...
def _last(seq, n): return list(seq)[-n:] if len(seq) > n else list(seq)
...
def compute_features(bars: List[Dict[str, float]]) -> Dict[str, Any]:
    ...
    def sma(arr,p): return mean(arr[-p:]) if len(arr)>=p else None
    ...
    return {
        "window": win, "slope_per_bar": slope, "volatility_stdev_returns": vola,
        "recent_high": max(highs[-win:]), "recent_low": min(lows[-win:]),
        "sma20": sma20, "sma50": sma50, "sma20_vs_50": cross,
    }
...
class ChatController(QObject):
    ...
    def on_user_message(self, user_text: str):
        ...
        context = {
            ...
            "recent_bars": [{"time": b["time"], "open": b["open"], "high": b["high"], "low": b["low"], "close": b["close"]}
                            for b in recent],
        }
        sys_prompt = (
